...
$(function() {
    ...
    $('#attachment').change(upload_attachment);
    
    $('body').on('click','.selectable-txt', function(e){
         $(this).select();
    });
    if(typeof (do_enable_rte)!='undefined') enable_rte(1);
    ...
    //pag_clone = $( ".pagination" ).clone();
    //$('.blocks_section').first().before(pag_clone);
    //$('.top-pagination').html(pag_clone);
...
//    $('.do-tooltip').tooltip({container: 'body'});
    ...
    //$('#next_item_btn:visible').effect("highlight", {}, 2000);
    // lesson validation
...
//        $('#mark-read-' + message).html('Mark as unattended');
//        $('#mark-read-' + message).attr('onclick', 'mark_unattended(' + message + ',' + block_id + ')');
//        $('#comment-' + message).removeClass('purple-border');
    });
}
...
function mark_unattended(message, block_id) {
    ...
    $.post(APP_URL + '/mark_unattended', {message: message, block_id: block_id}, function(result) {
        ...
        //$('#mark-read-'+message).parent().addClass('new_comment');
...
function mark_submission_attended(message) {
    ...
    $.post(APP_URL + '/mark_submission_attended', {message: message}, function(result) {
        hide_busy();
        $('#mark-s-read-' + message).html('Mark as unattended');
        ...
        //$('#mark-s-read-'+message).parent().removeClass('unattended_item');
        //$('#mark-s-read-'+message).parent().removeClass('unattended_item_visited');
        //$('#mark-s-read-'+message).parent().parent().removeClass('unattended_block');
        console.log('remove unattended');
...
function mark_submission_unattended(message) {
    ...
    $.post(APP_URL + '/mark_submission_unattended', {message: message}, function(result) {
        hide_busy();
        $('#mark-s-read-' + message).html('<i class="glyphicon glyphicon-ok"></i> Mark as attended');
        $('#mark-s-read-' + message).removeClass('red-button');
        $('#mark-s-read-' + message).addClass('greenyellow-button');
        // $('#mark-s-read-'+message).before(" <span id='unattended-warning-"+message+"'><i class='glyphicon glyphicon-exclamation-sign'></i> Submission Unattended</span> ");
        $('#mark-s-read-' + message).attr('onclick', 'mark_submission_attended(' + message + ')');
        //$('#mark-s-read-'+message).parent().addClass('unattended_item');

        //$('#mark-s-read-'+message).addClass('btn-success');
        //$('#mark-s-read-'+message).removeClass('btn-danger');
        //$('#mark-s-read-'+message).parent().parent().addClass('unattended_block');
        $('#next_item_btn').show();
    });
}
...
function do_reply(block_id) {
    ...
    show_busy();
    $('#reply_form').remove();
    $.post(APP_URL + '/reply', {block_id: block_id, reply_txt: reply_txt}, function(result) {
        if (result == 'error') {
        ...
        $('#comments-' + block_id).append($('.reply-comments-' + block_id));
        $('.reply-comments-' + block_id).attr('onclick', 'show_edit_form(' + block_id + ',' + result.id + ')');
        $('.reply-comments-' + block_id).show();
        $('.reply-comments-' + block_id).addClass('edit-comment');
...
function show_edit_form(block, id) {
    console.log($("#edit_reply").length);
    if ($("#edit_reply").length > 0) {
        $('.reply-comments-' + block).html('Edit Comment');
        $('.reply-comments-' + block).show();
        cancel_edit(block, id);
        return false;
    }
    $('.reply-comments-' + block).html('Cancel');
    $('.reply-comments-' + block).hide();
    $("#edit_reply").remove();
    $('#comment-' + id + ' .comment-content').hide();
    content = $('#comment-' + id + ' .content').html();
    str = '<div id="edit_reply" class="white-textarea-container comment-textarea"><textarea  id="edit_reply_txt" class="white-textarea">' + content + '</textarea><button type="button" class="buttons short-buttons cyan-button" onclick="do_comment_edit(' + block + ',' + id + ')">SUBMIT</button><button type="button" class="buttons short-buttons cyan-button cancel-button" onclick="show_edit_form(' + block + ',' + id + ')">CANCEL</button></div>';
    // $('#comment-'+id).append('<div id="edit_reply"><textarea class="form-control" id="edit_reply_txt">'+content+'</textarea><button type="button" class="btn btn-danger" onclick="do_comment_edit('+block+','+id+')">Edit</button> <button type="button" class="btn btn-danger" onclick="cancel_edit('+block+','+id+')">Cancel</button></div>');
    $('#comment-' + id).append(str);
    $('#edit_reply_txt').focus();
}

function cancel_edit(block, id) {
    $("#edit_reply").remove();
    $('#comment-' + id + ' .comment-content').show();
}
...
//    if (strip_tags($('#edit_reply_txt').code()) == '') {
//        do_growl('Cannot post empty comment', 'danger');
//        return false;
//    }
...
//        $("#edit_reply").remove();
//
//        $('#comments-' + block).append(result);
//        $('#comments-' + block).append($('.reply-comments-' + block));
//        $('.reply-comments-' + block).show();
        box = $('#comment-'+id);
...
function user_attended(user) {
    ...
    if (result == true) {
        ...
        $.post(APP_URL + '/mark_user', {user: user}, function(result) {
            ...
            do_growl('User has been marked as attended', 'success');
...
function post_coach_remarks(url) {
    if (strip_tags( $('#remark').code() ) == '') {
        //do_growl('Cannot post empty remark!', 'danger');
        $('#remark-warning').remove();
        //$('#post_remark_btn').after(' <p id="remark-warning">Cannot post empty remark!</p>');
        do_growl('Cannot post empty message!', 'danger');
        $('#remark').focus();
        setTimeout(hide_remark_warning, 1000);
        return false;
    }
    show_busy();
    remark =  $('#remark').code();
    $.post(url, {user: $('#remark_user').val(),
        ...
        result = parse_json(result);
        hide_busy();
        //cancel_coach_remarks();
        $('#remark-' + result.id).remove();
        $('#posted_remarks').append(result.html);
...
function do_remark_reply(lesson){
    reply_txt = $('#remark_reply').code();
...
function edit_remark(e){
    e.preventDefault();
    if($('.edit-remark-txt').length > 0) return false;
    id = $(e.target).attr('data-id');
    cnt = $(e.target).parent().parent().parent().find('.text');
    cnt.hide();
    //cnt.after('<div class="edit-remark-ui"><textarea class="summernote_editor edit-remark-txt">'+cnt.html()+'</textarea><button type="button" class="btn btn-default2 message-send" onclick="do_edit_remark('+id+')">Send</button></div>');
    $(e.target).parent().parent().parent().find('.attachment').after('<div class="edit-remark-ui"><textarea class="summernote_editor edit-remark-txt">'+cnt.html()+'</textarea><button type="button" class="btn btn-default2 message-send" onclick="do_edit_remark('+id+')">Send</button></div>');
...
function edit_comment(e, block, id){
    e.preventDefault();
    if($('.edit-comment-txt').length > 0) return false;
    cnt = $('#comment-'+id).find('.text');
    cnt.hide();
    $('#comment-'+id).find('.attachment').after('<div class="edit-comment-ui"><textarea class="summernote_editor edit-comment-txt">'+cnt.html()+'</textarea><button type="button" class="btn btn-default2 message-send" onclick="do_comment_edit(' + block + ',' + id + ')">Send</button></div>');
...
function upload_attachment(e){
    ...
    file = document.getElementById('attachment');
    if(file.value==''){
        hide_busy();
    ...
    if (!hasExtension('attachment', allowed)) {
        hide_busy();
        do_growl("This file type is not allowed.",'danger');
        return false;
    }
    ...
    if($('.alert-dismissible').length==0){
        $('.note-editor').last().after('<div class="alert-dismissible message-col-alert" role="alert"><button type="button" class="close close-2" data-dismiss="alert"></button>     <p></p></div>');
    }
    else{
        $('.alert-dismissible').last().after('<div class="alert-dismissible message-col-alert" role="alert"><button type="button" class="close close-2" data-dismiss="alert"></button>     <p></p></div>');
    }
    
    $box = $('.alert-dismissible').last();
