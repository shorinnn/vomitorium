...
if($answer!=null){
    ...
    $functional = $answer_str["functional"];
    $personality = $answer_str["personality"];
...
<input class='mc_validation' type="hidden" data-mc-block="{{$block->id}}-f"  data-mc-min="{{$block->minimum_choices}}" 
       ...
       data-mc-message="You must pick between  {{$block->minimum_choices}} and {{$block->maximum_choices}} functional skills for '{{$block->title}}'." />
@else
@endif
   ...
   <?php 
    ...
    ?>
    <div class="skill-dv"><input  {{disable_answered($answer_str)}}  {{$checked}}  type="checkbox" class='mc-answer-{{$block->id}}-f' name='skill-answer[{{$block->id}}][functional][]' value="{{$p}}" id="skill-{{$i}}" /> <label for="skill-{{$i}}">{{$p}}</label></div>
      @endforeach
	<span class="cl"></span>
</div>
...
<input class='mc_validation' type="hidden" data-mc-block="{{$block->id}}-p"  data-mc-min="{{$block->minimum_choices}}" 
       data-mc-max="{{$block->maximum_choices}}" 
       data-mc-message="You must pick between  {{$block->minimum_choices}} and {{$block->maximum_choices}} personality skills for '{{$block->title}}'." />
@else
...
@endif
   ...
   if($answer_str!=null){
    ...
    <div class="skill-dv"><input  {{disable_answered($answer_str)}}  {{$checked}} type="checkbox"  class='mc-answer-{{$block->id}}-p' id='skill-{{$i}}' name='skill-answer[{{$block->id}}][personality][]' value="{{$p}}" /> <label for="skill-{{$i}}">{{$p}}</label></div>
